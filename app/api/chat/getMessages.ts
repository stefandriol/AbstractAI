...
const getMessages = async (arxivCategory: string, interest: string): Promise<Message[]> => {
    ...
    const papersForPrompt: PaperForPrompt[] = papers.slice(0, 10).map(({id, ...rest}) => rest);

    const messages: Message[] = [];

    messages.push({
        role: ChatCompletionRequestMessageRoleEnum.System,
        content: "Join the academic community in summarising new high energy physics papers."
    });
    ...
    const interestPrompt: string = interest ? `Only summarize the papers that are strictly relevant for my interests: ${interest}${interest.slice(-1) === '.' ? ' ' : '. '}` : '';
