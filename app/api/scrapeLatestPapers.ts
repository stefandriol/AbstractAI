...
export async function scrapeLatestPapers(arxivCategory: string): Promise<{ id: string, title: string; authors: string; abstract: string }[]> {
  try {
    ...
    const papers: { id: string; title: string; authors: string; abstract: string }[] = [];
    ...
    dtElements.each((index, dtElement) => {
        ...
        const title = titleElement.text().replace('Title:', '').replace(/\s*\n\s*/g, ' ').replace(/  +/g, ' ').trim();
        const authorsElement = $(dtElement).next().find('.list-authors');
        const authors = authorsElement.text().replace('Authors:', '').replace(/\s*\n\s*/g, ' ').replace(/  +/g, ' ').trim();
        const abstractElement = $(dtElement).next().find('p.mathjax');
        const abstract = abstractElement.text().replace(/\s*\n\s*/g, ' ').replace(/  +/g, ' ').trim();
        papers.push({
            id: id,
            title: title,
            authors: authors,
